...
class MultipartQuestionSubQuestionsEdit extends Component
{
    ...
    public function editQuestion(): void
    {
        ...
        $question = ExamService::editSubQuestion($this->question->id, $this->question_form->title);
        switch ($this->question_form->question_type) {
            case 'multiple_choice':
                ...
                foreach ($options as $index => $option) {
                    $i = $index + 1;
                    $field = 'option' . $i;
                    ExamService::editSubQuestionOption($option->id, $this->question_form->$field, $this->question_form->correct_answer == $i ? true : false);
                }
                break;
    ...
    {
        $this->redirect(route('management.courses.questions.sub-questions', [
    ...
    {
        $this->question = SubQuestion::findOrFail($question_id);
    ...
    public function render(): View|Application|Factory|\Illuminate\View\View
    {
        ...
        return view('livewire.management.questions.manage-questions.multipart-question-sub-questions-edit');
    }
}
